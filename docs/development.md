...
The tagged-release workflow is dormant until a `v*` tag is pushed. It rejects a tag that does not equal `v` plus the Cargo package version or whose commit is not on `main`. Successful tags build six native release archives, generate SHA-256 checksums, and create a GitHub Release from the existing tag. Prepare a release by updating `Cargo.toml` and `CHANGELOG.md`, committing those changes, creating an annotated matching tag, and pushing that tag. Crates.io publication remains a separate explicit action.
