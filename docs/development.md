...
Search text is normalized once per record. Its compact key retains alphanumeric characters from every script while folding common Latin diacritics and ignoring punctuation and Unicode whitespace; the separate sort key preserves punctuation and spacing. Global search uses Rust's short-string substring search because the real dataset's individual name-and-ID haystacks are too small to amortize SIMD dispatch; searchable checkbox lists use `memchr::memmem::Finder` across their longer option scan. Ascending and descending `LocationId` orders for every table column are generated with the committed asset bundle. A filter or sort interaction therefore scans one already ordered bounded index and does not perform an interaction-time sort or grow a cache.
